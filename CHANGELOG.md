...
### Features
- **Cookie consent for Google Analytics**. Google Analytics no longer loads unconditionally on first visit. A consent dialog now asks before any tracking starts, is revisitable anytime from Settings, and declining tears down the GA script and any GA cookies already set.
